...
void DynWbc::_SetOptimizationSize() {
  ...
  if (_dim_rf > 0) {
    ...
    _dyn_cilb = DVec(_dim_Uf);
    _dyn_ciub = DVec(_dim_Uf);
    ...
    _Uf_ieq_vec_lb = DVec(_dim_Uf);
    _Uf_ieq_vec_ub = DVec(_dim_Uf);
  } else {
    _dyn_CI = DMat::Zero(1, _dim_opt);
    _dyn_cilb = DVec::Zero(1); // may be error
    _dyn_ciub = DVec::Zero(1); // may be error
  }
}
...
void DynWbc::_ContactBuilding() {
  ...
  (*_contact_list)[0]->getRFConstraintVecLb(Uf_ieq_vec_lb);
  (*_contact_list)[0]->getRFConstraintVecUb(Uf_ieq_vec_ub);
  ...
  _Uf_ieq_vec_ub.head(dim_accumul_uf) = Uf_ieq_vec_ub;
  ...
  for (int i = 1; i < (*_contact_list).size(); ++i) {
    ...
    (*_contact_list)[i]->getRFConstraintVecLb(Uf_ieq_vec_lb);
    (*_contact_list)[i]->getRFConstraintVecUb(Uf_ieq_vec_ub);
    // std::cout << "Uf_ieq_vec_lb\n" << Uf_ieq_vec_lb << std::endl;
    // std::cout << "Uf_ieq_vec_ub\n" << Uf_ieq_vec_ub << std::endl;
    _Uf_ieq_vec_lb.segment(dim_accumul_uf, dim_new_uf) = Uf_ieq_vec_lb;
    _Uf_ieq_vec_ub.segment(dim_accumul_uf, dim_new_uf) = Uf_ieq_vec_ub;
...
void DynWbc::_SetInEqualityConstraint() {
  ...
  _dyn_cilb = _Uf_ieq_vec_lb - _Uf * _Fr_des;
  _dyn_ciub = _Uf_ieq_vec_ub - _Uf * _Fr_des;
}
...
void DynWbc::_SolveQP() {
  ...
  */
  ...
  DVec lower_bound(_dyn_ce0.rows() + _dyn_cilb.rows());
  DVec upper_bound(_dyn_ce0.rows() + _dyn_cilb.rows());
  lower_bound.segment(0, _dyn_ce0.rows()) = _dyn_ce0;
  upper_bound.segment(0, _dyn_ce0.rows()) = _dyn_ce0;

  lower_bound.segment(_dyn_ce0.rows(), _dyn_cilb.rows()) = _dyn_cilb;
  upper_bound.segment(_dyn_ce0.rows(), _dyn_cilb.rows()) = _dyn_ciub;
