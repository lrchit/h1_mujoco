...
template <typename T> inline bool IsDifferent(const T &a, const T &b) {
...
template <typename T> inline void CopyScalar(T &dst, const T &src) {
  dst = src;
}
...
template <typename T> inline void Copy(T &dst, const T &src) {
  if constexpr (std::is_array_v<T>) {
    CopyArray(dst, src);
  } else {
    CopyScalar(dst, src);
...
// operations which require holding the mutex, prevents racing with physics
// thread
void Simulate::Sync() {
